...
# on repondère les variables en ajoutant un poids dans les membres de l'équation
# ici j'ai choisi 1/deptot ce qui revient à repondérer les données par 1/racine(deptot)
...
### test: on voit que cela ne marche pas.
ols_test_breusch_pagan(E.loisirs.pca.wls)
ols_test_f(E.loisirs.pca.wls)
ols_test_score(E.loisirs.pca.wls)

### prochaine séance sur ces données donc: traiter l'hétéroscédasticité
### par deux moyens: (1) en utilisant l'estmation des MCG (moindres carrés généralisés)
### cela consiste à retirer l'hétéroscédasticité en stabilisant la variance du résidu
### par un rééchelonnement du résidu.
### (2) utiliser une variable liée au résidu pour repondérer les données.
### Ici comme (2) ne semble pas marcher on testera donc (1)

### Partie 3 : bricole'R 2019.3 ----
### Nous explorons ici l'évolution des dépenses contraintes
### data import via readxl: read_excel("data_depcontr.xlsx")
### in the current directory of bricol'R
### let(data_depcontr) us examine the content of the new data set data_depcontr:
attributes(data_depcontr)
### a useful tip: how to show the loaded package in your session:
(.packages())
### this section necessitates the sandwich and jtools libraries
library(jtools)
library(sandwich)
# we see there are 15 columns in $names, 58 rows in $row.names and three $class objects.
## let us explore this new data set
attach(data_depcontr)
## let us plot the share of constrained expenditures:
ggplot(data = data_depcontr, mapping = aes(x = Annee, y = Wdepcontr)) +  
  geom_point() + geom_line() +
  ggtitle("Evolution of constrained expenditure share") +
  labs(x = "Année", y = "Share of constrained households expenditure") +
  theme_bw() +
  scale_color_brewer(palette = "Paired")
## the same but in share of disposable income:
ggplot(data = data_depcontr, mapping = aes(x = Annee, y = Wdepcontrdb)) +  
  geom_point() + geom_line() +
  ggtitle("Evolution of constrained expenditure share relative to disposable income") +
  labs(x = "Année", y = "Share of constrained households expenditure") +
  theme_bw() +
  scale_color_brewer(palette = "Paired")

### A first esimtation with deflated income by Stone index and constrained expenditures price
WcontrSt10 <- lm(Wdepcontr ~ trend + lnRDBSt10 + log(IPCONTR10))
### the standard summary:
summary(WcontrSt10)

### the improved summary by jstools:
summ(WcontrSt10, vifs = TRUE, part.corr = TRUE)
# same with 5% condidence intervals for coefficients:
summ(WcontrSt10, confint = TRUE)
## the VIFs hint at a very strong colinearity between ln RDBSt10 and trend. 
## let us check that with olsrr colinearity diagnostics:
ols_correlations(WcontrSt10)
### confirm with olsrr plots
ols_plot_diagnostics(WcontrSt10)
### let us check the regular plot diags:
plot(WcontrSt10)


## let us check which points are leverages:
## 
### calcul du levier: créer objet et le représenter dans un ggplot
ols_leverage(WcontrSt10)
lev.WcontrSt10 <- ols_leverage(WcontrSt10)
ggplot(data = data_depcontr) +  
  geom_point(mapping = aes(x = lev.WcontrSt10, y = WcontrSt10$fitted.values)) +
  geom_text(mapping = aes(x = lev.WcontrSt10, y = WcontrSt10$fitted.values, label = Annee, 
                          hjust = 0), color = "blue", size = 3) +
  labs(x = "lever", y = "fitted share of constrained expenditures") +
  theme_bw() +
  scale_color_brewer(palette = "Paired")
### the same but with residuals not fitted valeues
ggplot(data = data_depcontr) +  
  geom_point(mapping = aes(x = lev.WcontrSt10, y = WcontrSt10$residuals)) +
  geom_text(mapping = aes(x = lev.WcontrSt10, y = WcontrSt10$residuals, label = Annee, 
                          hjust = 0), color = "blue", size = 3) +
  labs(x = "lever", y = "residuals from fitted share of constrained expenditures") +
  theme_bw() +
  scale_color_brewer(palette = "Paired")
### improve the ggplot:
ggplot(data = data_depcontr, mapping = aes(x = lev.WcontrSt10, y = WcontrSt10$residuals)) +  
  geom_point() +
  geom_text(label = Annee, hjust = 0, color = "blue", size = 3) +
  labs(x = "lever", y = "residuals from fitted share of constrained expenditures") +
  theme_bw() +
  scale_color_brewer(palette = "Paired")
### see which value is most influential
ggplot(data = data_depcontr, mapping = aes(x = lev.WcontrSt10, y = Wdepcontr)) +  
  geom_point() + geom_line(color = "red") +
  geom_text(label = Annee, hjust = 0, color = "blue", size = 3) +
  labs(x = "lever", y = "share of constrained expenditures") +
  theme_bw() +
  scale_color_brewer(palette = "Paired")


### let get rid of the trend:
WcontrSt10 <- lm(Wdepcontr ~ lnRDBSt10 + log(IPCONTR10))
### the improved summary by jstools:
summ(WcontrSt10, vifs = TRUE, part.corr = TRUE)
## the VIFs do not show any colinearity, this will show in the diags. 
## let us check that with olsrr colinearity diagnostics:
ols_correlations(WcontrSt10)
### partial r's are far improved.
### confirm with olsrr plots:
ols_plot_diagnostics(WcontrSt10)
### we get less trouble and better fit but an outlier that is influent !
### improve the ggplot:
lev.2.WcontrSt10 <- ols_leverage(WcontrSt10)
ggplot(data = data_depcontr, mapping = aes(x = lev.2.WcontrSt10, y = WcontrSt10$residuals)) +  
  geom_point() +
  geom_text(label = Annee, hjust = 0, color = "blue", size = 3) +
  labs(x = "lever", y = "residuals from fitted share of constrained expenditures") +
  theme_bw() +
  scale_color_brewer(palette = "Paired")
### see which value is most influential
ggplot(data = data_depcontr, mapping = aes(x = lev.2.WcontrSt10, y = Wdepcontr)) +  
  geom_point() + geom_line(color = "red") +
  geom_text(label = Annee, hjust = 0, color = "blue", size = 3) +
  labs(x = "lever", y = "share of constrained expenditures") +
  theme_bw() +
  scale_color_brewer(palette = "Paired")
### clearly the high lever points are the first 4 last 2013--2016 !
### perhaps, reestimating the model without those 4 years could be a good idea.
### les us compute elasticities:
### calcul des élasticités
el.inc.depcontr <- 1 + WcontrSt10$coefficients[[2]]/WcontrSt10$fitted.values
el.prix.depcontr <- WcontrSt10$coefficients[[3]] / WcontrSt10$fitted.values - WcontrSt10$coefficients[[2]]
summary(el.inc.depcontr)
summary(el.prix.depcontr)



### add I() and computations into the expression
WcontrSt10_2 <- lm(Wdepcontr ~ trend + I(log(RDBnom) - ipstonecontr) + I(log(IPCONTR10)))
summary(WcontrSt10_2)
attributes(WcontrSt10_2)
View(WcontrSt10_2)
### load jtools
library(jtools)
summ(WcontrSt10_2)
